...
-- brain.hs and the resulting weights are embedded as compile-time constants.
--
-- Synthesise to VHDL:
...
-- Trained weights
--
-- Replace the placeholders below with values exported from brain.hs.
--
-- To convert a trained Double weight w to Weight use:
--   fromRational (toRational w) :: Weight
--
-- Example helper you can add to brain.hs to print Clash-compatible literals:
--
--   import Data.List (intercalate)
--   showVec :: [Double] -> String
--   showVec xs = "(" <> intercalate " :> " (map show xs) <> " :> Nil)"
-- ---------------------------------------------------------------------------

-- | All-zero layer used as a stand-in until real weights are embedded.
zeroLayer :: (KnownNat i, KnownNat o) => Layer i o
zeroLayer = (repeat 0, repeat (repeat 0))

-- | Replace this with your actual trained weights.
trainedBrain :: Brain4_3_2
trainedBrain = (zeroLayer, zeroLayer)
